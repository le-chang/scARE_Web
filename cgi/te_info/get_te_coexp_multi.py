...
feature2s=set(form['Gene'].value.split(','))
...
####
    exp2=None
    for gene in feature2s:
        cursor.execute(f"select TABLE_ID from GENE_DICT WHERE GENE='{gene}' ")
        table_id2=cursor.fetchone()

        if table_id2:
            table_id2=table_id2[0]
            sql=f'''SELECT CELL,scARE_ID,{gene} from CELL_EXP_{table_id2} WHERE CELL_EXP_{table_id2}.scARE_ID = "{dataset}"'''
            cursor.execute(sql)
            info=cursor.fetchall()
            if exp2 is None:
                exp2=pd.DataFrame(info)
                exp2.columns=['CELL','scARE_ID',gene]
            else:
                tmp=pd.DataFrame(info)
                tmp.columns=['CELL','scARE_ID',gene]
                exp2=pd.merge(exp2,tmp,on=['CELL','scARE_ID'])
            exp2[gene]=numpy.expm1(exp2[gene]+1)
    if exp2 is None:
        print(json.dumps(['No data','No data']))
    
    exp2['mean_feature2']=numpy.log1p(exp2.iloc[:,2:].mean(axis=1))
    exp2=exp2[['CELL','scARE_ID','mean_feature2']]
####
    ...
    dots=json.dumps(df.loc[:,[feature1,'mean_feature2']].to_numpy(), cls=NumpyArrayEncoder)

    ## get boxplot
    boxplot=[{'name':'Most expressed TE','data':[]}]
    min_,q1,median,q3,max_=df['mean_feature2'].describe()[['min','25%','50%','75%','max']].values
    ...
    labels=[feature1,'Mean expression of genes']

    ## get stats
    model = LinearRegression()
    model.fit(df[feature1].values.reshape(-1, 1), df['mean_feature2'].values.reshape(-1, 1))
    r_sq = model.score(df[feature1].values.reshape(-1, 1), df['mean_feature2'].values.reshape(-1, 1))
    intercept, slope = model.intercept_, model.coef_

    ranksums_=ranksums(df[feature1],df['mean_feature2'])
    pearsonr_=pearsonr(df[feature1],df['mean_feature2'])
