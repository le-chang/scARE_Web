...
try:
    ...
    sql="select TE,COMPARISON,DATASET,CELL,`CONDITION`,MEAN1,MEAN2,`FC`,PVAL,PCE1,PCE2 from EXP_DE where COMPARISON = '{}' and `CONDITION`='{}';".format(compare,disease)
    cursor.execute(sql)
    ret=cursor.fetchall()
    ...
    for i in range(len(ret)):
        ...
        pval='{:.2e}'.format(pval)
        qval='{:.2e}'.format(qval)
        table_row += row_fmt.format(te=te,dataset=dataset,poe1=poe1,poe2=poe2,cell1_median=cell1_median,cell2_median=cell2_median,fc=fc,pval=pval)
