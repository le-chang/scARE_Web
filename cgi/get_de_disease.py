...
try:
    ...
    sql="select TE,COMPARISON,DATASET,CELL,`CONDITION`,MEAN1,MEAN2,`FC`,PVAL,PCE1,PCE2 from EXP_DE where COMPARISON = '{}' and `CELL`='{}';".format(compare,cell)
    cursor.execute(sql)
    ret=cursor.fetchall()
    ret=pd.DataFrame(ret)
    ...
    for i in range(len(ret)):
        ...
        pval='{:.2e}'.format(pval)
        qval='{:.2e}'.format(qval)
        table_row += row_fmt.format(te=te,dataset=dataset,poe1=poe1,poe2=poe2,cell=cell,disease_median=disease_median,control_median=control_median,fc=fc,pval=pval)
