...
try:
  ...
  <div class="card mb-0" id="{dataset}"  >
      <div class="card-header" id="{dataset}_head">
        <h5 class="mb-0">
          ...
          <a href="dataset_detail.html?Cate=Dataset&KW={dataset}&CL=F" target="_blank"  aria-label="{dataset}">
  ...
  for dataset in info:
      dataset=dataset[0]
      dataset_info=f'<iframe class="border-0" width="100%" height="850px" {src}="dataset_detail.html?Cate=Dataset&KW={dataset}&CL=T" aria-label="{dataset}"></iframe>'
