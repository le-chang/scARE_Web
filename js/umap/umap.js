Highcharts.setOptions({
  ...
  getCellUMAPData().then(data => {
    const parseCellUMAP = CellType => {
      ...
      data.forEach(elm => {
        if (elm.CELL_TYPE === CellType) {
          temp.push({ x:elm.UMAP_1,y:elm.UMAP_2,cell_id:elm.CELL,disease:elm.DISEASE});
        }
      });
      return temp;
    };
    ...
    umap_chart = Highcharts.chart('container_umap', {
      ...
      tooltip: {
        pointFormat: 'Cell: {point.cell_id} <br/> UMAP_1: {point.x} <br/> UMAP_2: {point.y} </br> Diagnosis: {point.disease} </br> '
      },
      series
    });
